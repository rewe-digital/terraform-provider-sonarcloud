...
func TestAccUserGroup(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccUserGroupConfig(resourceName, names[0], descriptions[0]),
			...
			{
				Config: testAccUserGroupConfig(resourceName, names[0], descriptions[1]),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("sonarcloud_user_group.test_group", "name", names[0]),
			...
			{
				Config: testAccUserGroupConfig(resourceName, names[1], descriptions[1]),
				Check: resource.ComposeTestCheckFunc(
...
func testAccUserGroupConfig(resourceName string, name string, description string) string {
	return fmt.Sprintf(`
