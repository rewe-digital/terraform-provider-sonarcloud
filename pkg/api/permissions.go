...
type PermissionsAddGroup struct {
	...
	Organization string `json:"organization"` // Key of organization, used when group name is set
	Permission   string `json:"permission"`   // Permission<ul><li>Possible values for global permissions: admin, profileadmin, gateadmin, scan, provisioning</li><li>Possible values for project permissions admin, codeviewer, issueadmin, securityhotspotadmin, scan, user</li></ul>
	ProjectId    string `json:"projectId"`    // Project id
	ProjectKey   string `json:"projectKey"`   // Project key
}
...
type PermissionsAddGroupToTemplate struct {
	...
	TemplateName string `json:"templateName"` // Template name
}
...
type PermissionsBulkApplyTemplate struct {
	AnalyzedBefore    string `json:"analyzedBefore"`    // Filter the projects for which last analysis is older than the given date (exclusive).<br> Either a date (server timezone) or datetime can be provided.
	OnProvisionedOnly string `json:"onProvisionedOnly"` // Filter the projects that are provisioned
	Organization      string `json:"organization"`      // Key of organization, used when group name is set
	Projects          string `json:"projects"`          // Comma-separated list of project keys
	Q                 string `json:"q"`                 // Limit search to: <ul><li>project names that contain the supplied string</li><li>project keys that are exactly the same as the supplied string</li></ul>
	Qualifiers        string `json:"qualifiers"`        // Comma-separated list of component qualifiers. Filter the results with the specified qualifiers. Possible values are:<ul><li>TRK - Projects</li></ul>
	TemplateId        string `json:"templateId"`        // Template id
	TemplateName      string `json:"templateName"`      // Template name
}
...
type PermissionsCreateTemplate struct {
	...
	Organization      string `json:"organization"`      // Key of organization, used when group name is set
	ProjectKeyPattern string `json:"projectKeyPattern"` // Project key pattern. Must be a valid Java regular expression
}
...
type PermissionsSearchProjectPermissions struct {
	...
	Q          string `json:"q"`          // Limit search to: <ul><li>project names that contain the supplied string</li><li>project keys that are exactly the same as the supplied string</li></ul>
	Qualifier  string `json:"qualifier"`  // Project qualifier. Filter the results with the specified qualifier. Possible values are:<ul><li>TRK - Projects</li></ul>
}
...
type PermissionsSearchTemplates struct {
	Organization string `json:"organization"` // Key of organization, used when group name is set
	Q            string `json:"q"`            // Limit search to permission template names that contain the supplied string.
}
